...
interface RightsIssuePageContent {
  id?: number;
  slug: string;
  title: string;
...
export default function InvestorRightsIssueCMS() {
  const [pageContent, setPageContent] = useState<RightsIssuePageContent>({
    slug: 'rights-issue',
    title: 'Rights Issue',
    hasYearFilter: false,
    filterItems: [],
  ...
  const handlePdfUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Upload to rights-issue subfolder
  ...
  const loadPageContent = async () => {
    try {
      ...
      if (err.message && !err.message.includes('Backend server not available')) {
        setError(err.message || 'Failed to load Rights Issue page');
      }
  ...
  const handleSave = async () => {
    try {
      ...
      let existingPage;
      try {
        existingPage = await investorsCmsApi.getPageContentBySlug('rights-issue');
      } catch (e) {
      ...
      if (existingPage && existingPage.id) {
        ...
        setSuccess('Rights Issue page updated successfully');
      } else {
        await investorsCmsApi.createPageContent(pageContent);
        setSuccess('Rights Issue page created successfully');
      }
      ...
      setError(err.message || 'Failed to save Rights Issue page');
    }
  };
  ...
  const handleDeleteSection = (sectionIndex: number) => {
    if (!confirm('Are you sure you want to delete this section? All documents in this section will be deleted.')) {
      return;
    }
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="text-center py-12">
          ...
          <p className="mt-2 text-gray-600">Loading Rights Issue page...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-900">Rights Issue Page CMS</h2>
        <p className="text-sm text-gray-600 mt-1">Manage sections and documents for the Rights Issue page</p>
      </div>
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold text-gray-900">Sections ({pageContent.sections.length})</h3>
          <button
              ...
              <div key={sectionIndex} className="border border-gray-200 rounded-lg p-4">
                ...
                <div className="mb-4">
                  ...
                  {section.documents.length === 0 ? (
                    <div className="text-sm text-gray-500 py-2">No documents in this section.</div>
                  ) : (
      ...
      {showSectionForm && editingSection && editingSection.section && (
        <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
          <h4 className="text-md font-semibold text-gray-900 mb-4">
            {editingSection.sectionIndex >= 0 ? 'Edit Section' : 'Add New Section'}
          </h4>
          <div className="grid grid-cols-1 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g., Issue Documents"
              />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="e.g., Refex Draft Letter of Offer"
              />
            </div>
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-2">
              ...
              <div className="flex items-center gap-4 mb-2">
                <input
                  ...
                  className="hidden"
                  id="rightsIssuePdfUpload"
                  disabled={uploadingPdf}
                />
                <label
                  htmlFor="rightsIssuePdfUpload"
