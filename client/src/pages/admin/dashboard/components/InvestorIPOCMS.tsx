...
export default function InvestorIPOCMS() {
  ...
  const handlePdfUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Upload IPO PDFs to ipo subfolder
  ...
  const loadPageContent = async () => {
    try {
      ...
      if (err.message && !err.message.includes('Backend server not available')) {
        setError(err.message || 'Failed to load IPO page');
      }
  ...
  const handleSave = async () => {
    try {
      ...
      let existingPage;
      try {
        existingPage = await investorsCmsApi.getPageContentBySlug('ipo');
      } catch (e) {
      ...
      if (existingPage && existingPage.id) {
        ...
        setSuccess('IPO page updated successfully');
      } else {
        await investorsCmsApi.createPageContent(saveData);
        setSuccess('IPO page created successfully');
      }
      ...
      setError(err.message || 'Failed to save IPO page');
    }
  };
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="text-center py-12">
          ...
          <p className="mt-2 text-gray-600">Loading IPO page...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-900">IPO Page CMS</h2>
        <p className="text-sm text-gray-600 mt-1">Manage documents for the IPO page</p>
      </div>
