...
export default function InvestorStockExchangeQuarterlyComplianceCMS() {
  ...
  const [editingDocument, setEditingDocument] = useState<{ document: Document | null; documentIndex: number; sectionIndex: number } | null>(null);
  ...
  const handleSaveDocument = () => {
    ...
    const updatedSections = [...pageContent.sections];
    const section = updatedSections[editingDocument.sectionIndex];
    
    if (editingDocument.documentIndex >= 0) {
      // Update existing document
      section.documents[editingDocument.documentIndex] = doc;
    } else {
      // Add new document
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-900">Stock Exchange Quarterly Compliance Page CMS</h2>
        <p className="text-sm text-gray-600 mt-1">Manage sections and documents for the Stock Exchange Quarterly Compliance page</p>
      </div>
          ...
          <div className="space-y-4">
            {pageContent.sections.map((section, sectionIndex) => (
              <div key={sectionIndex} className="border border-gray-200 rounded-lg p-4">
                <div className="flex justify-between items-center mb-4">
                  <h4 className="text-md font-semibold text-gray-900">{section.title} ({section.documents.length} documents)</h4>
                  <div className="flex gap-2">
                    <button
            ...
            <div className={pageContent.hasYearFilter ? '' : 'md:col-span-2'}>
              ...
              <div className="flex items-center gap-4 mb-2">
                <input
                  ...
                  className="hidden"
                  id="stockExchangeCompliancePdfUpload"
                  disabled={uploadingPdf}
                />
                <label
                  htmlFor="stockExchangeCompliancePdfUpload"
